...
process {
    ...
    $SubComponentResourceResponse = Invoke-RestMethod -Authentication OAuth `
    ...
    if ($SubComponentResourceResponse.error) {
        $DataverseApiUri = New-Object uri $DataverseApiBase, "EntityDefinitions(LogicalName='${EntityLogicalName}')/${SubComponentType}"
        if ($VerbosePreference -ne 'SilentlyContinue') {
